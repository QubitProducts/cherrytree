...
Cherrytree.prototype.initialize = function (options) {
  ...
  this.logError = this.options.logError || function (err) {
  ...
  _.each(this.matchers, function (matcher) {
    if (!found) {
      ...
      if (params) {
        ...
        params.queryParams = Path.extractQuery(path) || {}
      }
    }
  })
  return {
...
Cherrytree.prototype.dispatch = function (path) {
  ...
  var transition = this.state.activeTransition = {
    ...
    prevParams: router.state.params || {},
    prevPath: router.state.path || '',
    routes: routes,
    params: params,
    path: path,
    cancel: function (err) {
  ...
  function callNext (i, prevResult) {
    ...
    if (i < router.middleware.length) {
      ...
      let middlewarePromise = router.middleware[i](transition, prevResult)
      try {
...
_.extend(Cherrytree.prototype, {
  ...
  generate: function (name, params) {
    ...
    var matcher, currentParams, pattern, paramNames, args, queryParams
    queryParams = {}
    ...
    if (matcher) {
      ...
      if (_.isObject(params)) {
        ...
        if (_.last(args) && _.last(args).queryParams) {
          queryParams = _.last(args).queryParams
          args.splice(-1)
        }
      ...
      var url = Path.injectParams(matcher.path, currentParams)
      url = Path.withQuery(url, queryParams || {})
