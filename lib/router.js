...
export default class Cherrytree {
  ...
  use (createMiddleware) {
    invariant(typeof createMiddleware === 'function', 'Middleware creator must be a function', createMiddleware)
    ...
    if (!middleware.name) {
      middleware.name = createMiddleware.name || (middleware.next && middleware.next.name)
    }
