...
export default function createTransition (options, Promise) {
  ...
  function runNext () {
    ...
    reduce(middlewares, function (context, middleware, i, list, cb) {
      ...
      log('Transition #' + id, 'resolving next:', name(middleware, 'next'))
      ...
      hook(middleware, 'next', next)(transition.descriptor, redirect, cancel, router)
    }, undefined, afterNext)
  }
  ...
  function runDone () {
    ...
    reduce(middlewares, function (context, middleware, i, list, cb) {
      log('Transition #' + id, 'resolving done:', name(middleware, 'done'))
      if (!middleware.done) return cb()
      let next = cb
      hook(middleware, 'done', next)(transition.descriptor, router)
  ...
  function runError (err) {
    let middlewares = router.middleware.slice(0, transition.middlewareReached + 1).reverse()
    reduce(middlewares, function (context, middleware, i, list, cb) {
      log('Transition #' + id, 'resolving error:', name(middleware, 'error'))
      if (!context) return cb(null)
      if (!middleware.error) return cb(null, context)
      let next = err => cb(null, err)
      hook(middleware, 'error', next)(context, transition.descriptor, router)
